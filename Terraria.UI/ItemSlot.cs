...
namespace Terraria.UI
{
	public class ItemSlot
	{
		...
		public static void Draw(SpriteBatch spriteBatch, Item[] inv, int context, int slot, Vector2 position, Color lightColor = default(Color))
		{
			...
			else if (item.type > 0 && item.stack > 0 && Options.HighlightNewItems && item.newAndShiny && context != 13 && context != 21 && context != 22)
			{
			...
			if (item.type > 0 && item.stack > 0)
			{
				...
				spriteBatch.Draw(texture2D3, position2, rectangle2, item.GetAlpha(currentColor), 0f, origin, num3 * scale, SpriteEffects.None, 0f);
				if (item.color != Color.Transparent)
				{
					spriteBatch.Draw(texture2D3, position2, rectangle2, item.GetColor(color), 0f, origin, num3 * scale, SpriteEffects.None, 0f);
				}
				if (item.stack > 1)
				{
		...
		public static void MouseHover(Item[] inv, int context = 0, int slot = 0)
		{
			...
			if (inv[slot].type > 0 && inv[slot].stack > 0)
			{
				...
				if (inv[slot].stack > 1)
				{
					...
					Main.hoverItemName = hoverItemName + " (" + inv[slot].stack + ")";
				}
