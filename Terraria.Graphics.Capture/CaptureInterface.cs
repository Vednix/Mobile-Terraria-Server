	...
	internal class CaptureInterface
	{
		...
		private class ModeEdgeSelection : CaptureInterfaceMode
		{
			...
			private void DrawMarkedArea(SpriteBatch sb)
			{
				...
				GetMapCoords(num, num2, 1, out Point result2);
				GetMapCoords(num + num3 + 1, num2 + num4 + 1, 1, out Point result3);
				...
				Rectangle.Intersect(ref value4, ref value3, out Rectangle result4);
		...
		private class ModeDragBounds : CaptureInterfaceMode
		{
			...
			private void DragBounds(Vector2 mouse)
			{
				...
				{
					GetMapCoords(num, num2, 1, out Point result3);
					GetMapCoords(num + num3 + 1, num2 + num4 + 1, 1, out Point result4);
				...
				if (currentAim != -1)
				{
					...
					{
						if (!GetMapCoords((int)mouse.X, (int)mouse.Y, 0, out Point result5))
						{
							return;
						}
						point = result5;
