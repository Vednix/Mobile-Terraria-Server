	...
	public class Netplay
	{
		...
		private static void OpenPort()
		{
			...
			if (mappings != null)
			{
				...
				if (!portForwardOpen)
				{
					mappings.Add(portForwardPort, "TCP", portForwardPort, portForwardIP, bEnabled: true, "Terraria Server");
					portForwardOpen = true;
				}
			}
		}
				...
				for (int k = 0; k < 17; k++)
				{
					...
					else if (Clients[k].IsConnected())
					{
						...
						if (Clients[k].StatusMax > 0 && Clients[k].StatusText2 != "")
						{
							...
							{
								Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " " + Clients[k].StatusText2 + ": Complete!";
							...
							{
								Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " " + Clients[k].StatusText2 + ": " + (int)((float)Clients[k].StatusCount / (float)Clients[k].StatusMax * 100f) + "%";
							}
						}
						else if (Clients[k].State == 0)
						{
							Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " is connecting...";
						}
						else if (Clients[k].State == 1)
						{
							Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " is sending player data...";
						}
						else if (Clients[k].State == 2)
						{
							Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " requested world information";
						}
						else if (Clients[k].State != 3 && Clients[k].State == 10)
						{
							try
							{
								Clients[k].StatusText = "(" + Clients[k].Socket.GetRemoteAddress() + ") " + Clients[k].Name + " is playing";
							}
							catch (Exception)
							{
								Clients[k].PendingTermination = true;
