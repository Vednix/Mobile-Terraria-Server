	...
	public class PortalHelper
	{
		...
		public static void TryGoingThroughPortals(Entity ent)
		{
			...
			for (int i = 0; i < FoundPortals.GetLength(0); i++)
			{
				...
				for (int j = 0; j < 2; j++)
				{
					...
					GetPortalEdges(projectile.Center, projectile.ai[0], out Vector2 start, out Vector2 end);
		...
		public static int TryPlacingPortal(Projectile theBolt, Vector2 velocity, Vector2 theCrashVelocity)
		{
			...
			for (int i = 0; i < (flag ? 2 : EDGES.Length); i++)
			{
				...
				if (num2 > 0f && FindValidLine(position, (int)EDGES[i].Y, (int)(0f - EDGES[i].X), out Point bestPosition))
				{
			...
			if (num != 0)
			{
				...
				if (num3 > 0f && FindValidLine(position, -SLOPE_OFFSETS[num - 1].Y, SLOPE_OFFSETS[num - 1].X, out Point bestPosition2))
				{
		...
		{
			GetPortalEdges(position, angle, out Vector2 start, out Vector2 end);
			for (int i = 0; i < 1000; i++)
			{
				...
				GetPortalEdges(projectile.Center, projectile.ai[0], out Vector2 start2, out Vector2 end2);
