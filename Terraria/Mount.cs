		...
		public void UseDrill(Player mountedPlayer)
		{
			...
			for (int i = 0; i < drillMountData.beams.Length; i++)
			{
				...
				if (flag)
				{
					...
					{
						flag2 = false;
						mountedPlayer.Hurt(mountedPlayer.statLife / 2, -mountedPlayer.direction, pvp: false, quiet: false, Lang.deathMsg(mountedPlayer.name, -1, -1, -1, 4));
					}
					if (flag2)
					{
			...
			case 9:
			{
				...
				if (flag2)
				{
					if (_abilityCooldown == 0 && mountedPlayer.whoAmI == Main.myPlayer)
					{
						...
						StopAbilityCharge();
						UseAbility(mountedPlayer, vector5, toggleOn: false);
					}
					else
					{
						AimAbility(mountedPlayer, vector5);
