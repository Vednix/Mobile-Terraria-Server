...
namespace Terraria
{
	public class NetMessage
	{
		...
		public static void SendData(int msgType, int remoteClient = -1, int ignoreClient = -1, string text = "", int number = 0, float number2 = 0f, float number3 = 0f, float number4 = 0f, int number5 = 0, int number6 = 0, int number7 = 0)
		{
			if (Main.netMode != 0)
			{
				...
				lock (buffer[num])
				{
					...
					case 32:
					{
						Item item4 = Main.chest[number].item[(byte)number2];
						writer.Write((short)number);
						writer.Write((byte)number2);
						short value = (short)item4.netID;
						writer.Write((short)item4.stack);
		...
		public static void greetPlayer(int plr)
		{
			if (Main.motd == "")
			{
				SendData(25, plr, -1, Lang.mp[18] + " " + Main.worldName + "!", 16, 255f, 240f, 20f);
			}
			else
			{
