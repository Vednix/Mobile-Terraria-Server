...
namespace Terraria
{
	public static class Utils
	{
		...
		public static string[] WordwrapString(string text, SpriteFont font, int maxWidth, int maxLines, out int lineAmount)
		{
			...
			while (list2.Count > 0)
			{
				...
				if (text2 == "\n")
				{
					...
					array3[num2] = array2[(long)intPtr] + text2;
					if (num >= maxLines)
					{
						break;
					}
				...
				else if (flag)
				{
					...
					else
					{
						...
						array5[num4] = array4[(long)intPtr2] + text2 + str;
						flag = false;
						list2.RemoveAt(0);
					}
				}
				...
				else
				{
					...
					array7[num5] = array6[(long)intPtr3] + text2 + str;
					flag = false;
					list2.RemoveAt(0);
				}
			}
		...
		{
			return (seed * 25214903917L + 11) & 0xFFFFFFFFFFFF;
		}
		...
		public static void DrawLaser(SpriteBatch sb, Texture2D tex, Vector2 start, Vector2 end, Vector2 scale, LaserLineFraming framing)
		{
			...
			framing(0, vector, num, default(Rectangle), out float distanceCovered, out Rectangle frame, out Vector2 origin, out Color color);
