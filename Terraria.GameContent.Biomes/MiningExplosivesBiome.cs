...
namespace Terraria.GameContent.Biomes
{
	internal class MiningExplosivesBiome : MicroBiome
	{
		public override bool Place(Point origin, StructureMap structures)
		{
			...
			bool flag = true;
			flag &= WorldUtils.Find(new Point(origin.X - ((num > 0.0) ? 3 : (-3)), origin.Y - 3), Searches.Chain(new Searches.Down(10), new Conditions.IsSolid()), out Point result);
			if (!(flag & WorldUtils.Find(new Point(origin.X - ((num > 0.0) ? (-3) : 3), origin.Y - 3), Searches.Chain(new Searches.Down(10), new Conditions.IsSolid()), out Point result2)))
			{
				return false;
			}
			result.Y--;
